...
1. **Exec source** - Shell script outputs JSON, LivePage parses and displays it
...
> Build a service status page with LivePage. Use an exec source to run a shell script that outputs JSON with service name, status (operational/degraded/outage), latency, and uptime. Show services in a table with status indicators. Add an incident log with severity levels. Use semantic HTML.
