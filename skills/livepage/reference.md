...
Complete API reference for building LivePage applications.

## File Structure

A LivePage app is a markdown file with:
...
You can define data sources directly in the frontmatter, making `livepage.yaml` optional for single-file apps:
...
Frontmatter sources take precedence over `livepage.yaml` sources if both define the same name.
...
Connect to external data. Source must be defined in `livepage.yaml`.
...
## Source Configuration (livepage.yaml)
...
Run a LivePage application:

```bash
livepage serve myapp.md           # Serve single file
livepage serve ./myapp/           # Serve directory
livepage serve . --port 8080      # Custom port
```
...
```bash
livepage build myapp.md -o myapp
```
