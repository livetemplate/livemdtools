...
func generateMarkdownSourceCode(sourceName, file, anchor, siteDir, currentFile string, readonly bool) (string, error) {
	...
	if !readonly {
		...
		code.WriteString("\theaderPattern := regexp.MustCompile(`(?m)^(#{1,6})\\s+(.+?)\\s*\\{#` + regexp.QuoteMeta(anchorName) + `\\}\\s*$`)\n")
		code.WriteString("\tcontentStr := string(content)\n")
		code.WriteString("\n")
		code.WriteString("\tmatches := headerPattern.FindStringSubmatchIndex(contentStr)\n")
		...
		code.WriteString("\t}\n")
		code.WriteString("\n")
		code.WriteString("\t// Pattern to match task with this ID\n")
		...
		code.WriteString("\t}\n")
		code.WriteString("\n")
		code.WriteString("\t// Toggle the checkbox\n")
		code.WriteString("\tnewContent := taskPattern.ReplaceAllStringFunc(sectionContent, func(match string) string {\n")
		code.WriteString("\t\tsubmatches := taskPattern.FindStringSubmatch(match)\n")
		...
		code.WriteString("\t}\n")
		code.WriteString("\n")
		code.WriteString("\t// Pattern to match line with this ID\n")
	...
	code.WriteString("\tanchorName := strings.TrimPrefix(s.anchor, \"#\")\n")
	code.WriteString("\n")
	code.WriteString("\t// Pattern: ## Title {#anchor}\n")
	code.WriteString("\theaderPattern := regexp.MustCompile(`(?m)^(#{1,6})\\s+(.+?)\\s*\\{#` + regexp.QuoteMeta(anchorName) + `\\}\\s*$`)\n")
	code.WriteString("\n")
	code.WriteString("\tmatches := headerPattern.FindStringSubmatchIndex(content)\n")
	code.WriteString("\tif matches == nil {\n")
	code.WriteString("\t\ts.Data = []map[string]interface{}{}\n")
	code.WriteString("\t\treturn nil\n")
	code.WriteString("\t}\n")
	...
	code.WriteString("\t\tif id == \"\" {\n")
	code.WriteString("\t\t\tid = generateID()\n")
	code.WriteString("\t\t}\n")
	code.WriteString("\n")
